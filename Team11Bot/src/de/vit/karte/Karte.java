...
public class Karte implements Inavigierbar {
	...
	/**
	 * das eigentliche Spielfeld mit allen Feldern die erste Array-Dimension
	 * bezeichnet die x-Achse die zweite Array-Dimension bezeichnet die y-Achse
	 */
	private Feld[][] karte;
	/**
	 ...
	 * groeßer ist als der bisher gespeicherte formularZaehler. Sobald
	 * Sachbearbeiter gefunden wurde, wird maximaler Wert gesetzt und Methode wird
	...
	/**
	 * grafische Ausgabe der Karte inklusive Entfernungen als String (nur zu
	 * Debug-Zwecken)
	 * 
	 * @return
	 */
	 ...
	 * Methode anstatt eines Integer-Arrays direkt zwei int zu uebergeben. Daher
	 * wird hier vom Standard abgewichen, wonach Methoden, die mit Koordinaten
	 * arbeiten immer ein Array mit 2 int uebergeben wird
	 * 
	 ...
	 * 
	 * @param cellStatus soll den kompletten oder geslicten (ohne Entfernung anderer
	 *                   Bots) String, den die getCellStatus-Methoden ausgeben,
	 *                   entegegennehmen
	 * 
	 ...
	 * bestimmt. Wenn ein Formular oder ein Blatt erfolgreich aufgenommen wurde,
	 * wird der formularZaehler der aufgesammelten Formulare oder der blattZaehler
	 * erhoeht. Der blattZaehler wird verringert, wenn ein Blatt erfolgreich
	 * abgelegt wurde. Falls nach der Position gefragt wird, wird die gespeicherte
	 * aktuellePostion aktualisiert, sofern sie von der erhaltenen abweicht.
	 * 
	...
	public void aktualisierePosition(String lastActionsResult, String lastDoneAction) {
		...
		case "OK ": // Blatt abgelegt
			...
			break;
		// Position abgefragt
		default: // da das lastActionResult bei der Positionsabfrage dynamisch ist, verwenden wir
					// hier den default-case und eine if-Abfrage
					// die letze Aktion war nach der eigenen Position zu fragen und wir haben eine
					// positive Rueckmeldung erhalten
	 ...
	 * Blaetter, die von uns erfolgreich gekickt wurden, bekommen den Status
	 * gekickt.
	 * 
	...
	public void aktualisiereNorden(String northCellStatus, String lastActionsResult, String lastDoneAction) {
		...
		if (!northCellStatus.contains(nameFeldNord)) {
			if (northCellStatus.contains("FORM") && !(this.getFeld(northCellStatus.substring(0, 8))[0] == -1)) { 	// wenn
																													// Formular
																													// an
																													// anderer
																													// Stelle
			...
			if (neuesFeld instanceof Sachbearbeiter) {
				...
				if (sb.getSpielerId() == this.getSpielerId() && !this.getStatischeZiele().containsValue(nordKoordinaten)) {
					this.statischeZiele.put(northCellStatus.substring(0, 10), nordKoordinaten); // unseren SB der
																								// Abbildung
																								// statischeZiele
																								// hinzufuegen
				}
				...
				if (form.getSpielerId() == this.getSpielerId() && !this.getStatischeZiele().containsValue(nordKoordinaten)) {
					this.statischeZiele.put(northCellStatus.substring(0, 8), nordKoordinaten); 	// unser Formular der
																								// Abbildung
																								// statischeZiele
																								// hinzufuegen
				}
			...
			else if (neuesFeld instanceof Blatt && lastActionsResult.equals("OK NORTH") && lastDoneAction.equals("kick north")) {
	...
	public void aktualisiereOsten(String eastCellStatus, String lastActionsResult, String lastDoneAction) {
		...
		if (!eastCellStatus.contains(nameFeldOst)) {
			if (eastCellStatus.contains("FORM") && !(this.getFeld(eastCellStatus.substring(0, 8))[0] == -1)) { 	// wenn
																												// Formular
																												// an
																												// anderer
																												// Stelle
			...
			if (neuesFeld instanceof Sachbearbeiter) {
				...
				if (sb.getSpielerId() == this.getSpielerId() && !this.getStatischeZiele().containsValue(ostKoordinate)) {
					this.statischeZiele.put(eastCellStatus.substring(0, 10), ostKoordinate); 	// unseren SB der Abbildung
																								// statischeZiele
																								// hinzufuegen
				}
			}
				...
				Formular form = (Formular) neuesFeld;
				this.setFormularZaehler(form.getNr()); // formularZehler erhoehen (wenn moeglich)
				if (form.getSpielerId() == this.getSpielerId() && !this.getStatischeZiele().containsValue(ostKoordinate)) {
					this.statischeZiele.put(eastCellStatus.substring(0, 8), ostKoordinate); // unser Formular der
																							// Abbildung statischeZiele
																							// hinzufuegen
				}
			}

			else if (neuesFeld instanceof Blatt && lastActionsResult.equals("OK EAST") && lastDoneAction.equals("kick east")) {
				Blatt blatt = (Blatt) neuesFeld;
				blatt.setGekickt(true); // selbst gekicktes Blatt bekommt den Status gekickt
			}
		}
	...
	public void aktualisiereSueden(String southCellStatus, String lastActionsResult, String lastDoneAction) {
		...
		if (!southCellStatus.contains(nameFeldSued)) {
			if (southCellStatus.contains("FORM") && !(this.getFeld(southCellStatus.substring(0, 8))[0] == -1)) { 	// wenn
																													// Formular
																													// an
																													// anderer
																													// Stelle
			...
			if (neuesFeld instanceof Sachbearbeiter) {
				...
				if (sb.getSpielerId() == this.getSpielerId() && !this.getStatischeZiele().containsValue(suedKoordinate)) {
					this.statischeZiele.put(southCellStatus.substring(0, 10), suedKoordinate); 	// unseren SB der
																								// Abbildung
																								// statischeZiele
																								// hinzufuegen
				}
				...
				if (form.getSpielerId() == this.getSpielerId() && !this.getStatischeZiele().containsValue(suedKoordinate)) {
					this.statischeZiele.put(southCellStatus.substring(0, 8), suedKoordinate); 	// unser Formular der
																								// Abbildung
																								// statischeZiele
																								// hinzufuegen
				}
			...
			else if (neuesFeld instanceof Blatt && lastActionsResult.equals("OK SOUTH") && lastDoneAction.equals("kick south")) {
				Blatt blatt = (Blatt) neuesFeld;
				blatt.setGekickt(true); // selbst gekicktes Blatt bekommt den Status gekickt
			}
		}
	...
	public void aktualisiereWesten(String westCellStatus, String lastActionsResult, String lastDoneAction) {
		...
		if (!westCellStatus.contains(nameFeldWest)) {
			if (westCellStatus.contains("FORM") && !(this.getFeld(westCellStatus.substring(0, 8))[0] == -1)) { 	// wenn
																												// Formular
																												// an
																												// anderer
																												// Stelle
			...
			if (neuesFeld instanceof Sachbearbeiter) {
				Sachbearbeiter sb = (Sachbearbeiter) neuesFeld;
				this.setFormularZaehler(sb.getFormularZaehler()); // formularZaehler erhoehen
				if (sb.getSpielerId() == this.getSpielerId() && !this.getStatischeZiele().containsValue(westKoordinate)) {
					this.statischeZiele.put(westCellStatus.substring(0, 10), westKoordinate); 	// unseren SB der
																								// Abbildung
																								// statischeZiele
																								// hinzufuegen
				}
				...
				this.setFormularZaehler(form.getNr()); // formularZaehler erhoehen (wenn moeglich)
				if (form.getSpielerId() == this.getSpielerId() && !this.getStatischeZiele().containsValue(westKoordinate)) {
					this.statischeZiele.put(westCellStatus.substring(0, 8), westKoordinate); 	// unser Formular der
																								// Abbildung
																								// statischeZiele
																								// hinzufuegen
				}
			...
			else if (neuesFeld instanceof Blatt && lastActionsResult.equals("OK WEST") && lastDoneAction.equals("kick west")) {
				Blatt blatt = (Blatt) neuesFeld;
				blatt.setGekickt(true); // selbst gekicktes Blatt bekommt den Status gekickt
			}
		}
	...
	/**
	 * Methode, die den Feldstatus am aktuellen Standpunkt ueberprueft und
	 * aktualisert Diese Methode ist nur relevant, wenn ein Dokument auf ein Feld
	 * gekickt wird, auf das wir uns entschieden haben zu gehen oder wir ein Blatt
	 * erfolgreich gelegt haben
	 * 
	...
	public void aktualisiereStandpunkt(String currentCellStatus, String lastDoneAction) {
		...
		if (!currentCellStatus.contains(nameFeldUnterUns)) {
			if (currentCellStatus.contains("FORM") && !(this.getFeld(currentCellStatus.substring(0, 8))[0] == -1)) { 	// wenn
																														// Formular
																														// an
																														// anderer
																														// Stelle
			...
			if (neuesFeld instanceof Formular) {
				Formular form = (Formular) neuesFeld;
				this.setFormularZaehler(form.getNr()); // formularZaehler erhoehen (wenn moeglich)
				if (form.getSpielerId() == this.getSpielerId() && !this.getStatischeZiele().containsValue(aktuellePosition)) {
					this.statischeZiele.put(currentCellStatus.substring(0, 8), standPunktKoordinaten); 	// unser Formular
																										// der Abbildung
																										// statischeZiele
																										// hinzufuegen
				}
				...
				Blatt blatt = (Blatt) neuesFeld;
				blatt.setGekickt(true); // selbst gelegtes Blatt bekommt den Status gekickt (damit wir es nicht gleich
										// wieder kicken)
			}
		}
	}
	 ...
	 * Entfernungen der bereits bekannten Felder in der Karte anzupassen. So
	 * entsteht eine Entfernungskarte, welche unserem Bot die
	 * Entscheidungsfaehigkeit gibt, den schnellsten/kuerzesten Weg innerhalb eines
	 * bekannten Abschnittes zu gehen!
	 * 
	 * @authors Franz Bogmann
	 */
			...
			for (int x = 0; x < this.getSpielfeldAbmessung()[0]; x++) {
				for (int y = 0; y < this.getSpielfeldAbmessung()[1]; y++) {
					...
					if (!((this.getFeld(derzeitigeKoordinate) instanceof Nebel) // Weder ein Nebel, noch
						...
						letzteBesteEntfernung = this.dynamischesZielFinden(derzeitigeKoordinate, letzteBesteEntfernung);

						if (kleinsteEntfernungEinesNachbarn + 1 < this.getFeld(derzeitigeKoordinate).getEntfernung()) {
	...
	/**
	 * Diese Methode setzt ein dynamisches Ziel, das wie folgt definiert ist: das
	 * von uns aus gesehene naechste(naheliegendste) freie Feld mit einem Nebel
	 * Nachbarn ODER ein nicht gekicktes Feld, worauf ein Papier liegt, was
	 * ebenfalls nahe dran ist. Diese Methode wird bei einer
	 * Entfernungsaktualisierung mehrfach aufgerufen und liefert immer die
	 * optimalste Entfernung zurueck, um beim naechsten Aufruf Entfernungen
	 * vergleichen zu koennen.
	 * 
	 * @authors Alexander Schmidt
	 * @param koordinaten, die in der Schleife von entfernungenAktualisieren gerade
	 *                     zum betrachteten Feld gehoeren
	 * @param entfernung
	 * @return temporaereEntfernung, die verglichen werden soll; bleibt gleich, wenn
	 *         es keine bessere gibt
	 */
	...
	/*
	 * WorstCase, wir finden das Formular nicht und muessen von vorne Suchen, oder
	 * unsere Exploration ist zu Ende
	 */
